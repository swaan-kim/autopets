use super::{
    db_err, encode, now_ms, validate_id, ConnectionState, EventInput, EventKind, SessionState,
    Store,
};
...
#[serde(rename_all = "camelCase", default)]
pub(super) struct SupervisionRecord {
    #[serde(flatten)]
    pub view: SessionSupervision,
    records: Vec<AttentionRecord>,
    elapsed_alerted_turn: Option<String>,
}

impl SupervisionRecord {
    pub(super) fn for_snapshot(&self, now: u64) -> SessionSupervision {
    ...
    fn close_turn(&mut self, turn_id: &str) {
...
fn validate_configuration(criterion: &str, minutes: Option<u32>) -> Result<(), String> {
...
pub(super) fn validate_plan(plan: &PlanPayload) -> Result<(), String> {
...
impl Store {
    ...
    pub fn configure_task(&mut self, input: TaskConfiguration) -> Result<TaskConfigured, String> {
        ...
        self.with_session_transaction(&session_id,|store| {
            if prior.is_none() {
                store.configure_session(&session_id,&input.completion_criterion,input.intervention_mode,input.elapsed_alert_minutes)?;
                store.db.execute("INSERT INTO task_config_requests(request_id,session_id,turn_id,fingerprint,applied_at) VALUES(?1,?2,?3,?4,?5)",params![input.request_id,session_id,input.turn_id,fingerprint,now_ms()]).map_err(db_err)?;
            }
            store.db.execute("INSERT OR REPLACE INTO slots(slot,session_id) VALUES(?1,?2)",params![slot,session_id]).map_err(db_err)?;
            store.slots[slot]=Some(session_id.clone());
            Ok(TaskConfigured{ok:true,session_id:session_id.clone(),slot})
        })
    }
    ...
    pub(super) fn tick_supervision(&mut self, now: u64) -> Result<bool, String> {
        let due: Vec<_> = self
            .sessions
            .iter()
            .filter_map(|(id, rec)| {
