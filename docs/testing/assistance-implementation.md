...
> 기록 범위 · 2026-09-23: 아래는 main 기반 자동 도움·구조 정리 검수다. 당시의 DB 보존·이관 설명을 후속 프리셋의 저장 영역까지 일반화하지 않는다. 후속 PR에는 workflow-v1 테이블 추가가 있으며, 최신 검사·배포 방향과 당시 기록은 [진행 현황](../roadmap/project-status.md)에서 구분한다. 아래 검사 수치와 실환경 미검증 판정은 당시 증거로 유지한다.
