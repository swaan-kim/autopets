...
## 수정 설치본 검증과 현재 PC 차단 (2026-09-27)

**설치형 MVP는 미완료다.** 연결 버튼의 Windows 경로 오류를 수정한 설치본은 별도 Windows 검사를 통과했지만, 현재 PC의 앱 제어 정책이 새 설치 파일 실행을 차단했다. 새 설치본의 실제 연결 버튼·일반 입력 A/B·계획→구현·펫 상태·복구는 아직 검사하지 못했다. 아래 과거 설치본의 실행 성공을 수정본 성공으로 대체하지 않는다.

| 항목 | 증거와 판정 |
| --- | --- |
| 소스 | PR head `3126f9884bdefdf3cd11c6f4a424385370def83b`, 빌드 merge `5cfe09fdd3008c47bb848a2d77b50f4b959f27eb` |
| 파일 | `AutoPets_0.1.0_x64-setup.exe`, 243,494,781바이트, SHA256 `44a439dcb527c8474468bcfa7da7c8facc9a0e32322442980e6d01baeba8ecad`, NotSigned |
| 코드·설치 검사 | [36266376326](https://github.com/swaan-kim/autopets/actions/runs/36266376326) 성공. Node 208, 화면/빌드, Rust 128 passed / 2 ignored, 설치·명시적 연결·복구·제거. 새 Rust 회귀는 실제 Node로 한글·공백 경로의 연결/해제 진입점을 실행했다 |
| 동일 설치본 수명주기 | [36267474373](https://github.com/swaan-kim/autopets/actions/runs/36267474373) 성공. 재호출 1개 프로세스 유지, X 숨김과 복원, 종료 메뉴로 프로세스/포트/연결 파일 종료, 재시작·제거·재설치 후 기록/설정/위치 보존. 강제 종료 0회. 제거 후 실행 파일·설치 등록·바로가기·프로세스 잔여 0 |
| 별도 Windows 실측 | 설치 24.569초, 첫 화면 5.617초, 최초 통신 준비 4.669초, 재시작 통신 준비 1.132초, 재설치 통신 준비 1.370초. 해당 CI PC 값이며 사용자 PC의 실측으로 표시하지 않는다 |
| 현재 PC 설치 시도 | 정상 Windows 설치 실행을 한 번 요청했으나 약 9.956초 후 `애플리케이션 제어 정책에서 이 파일을 차단했습니다`로 거절. 같은 파일의 CodeIntegrity 3033/3077 확인. Smart App Control On, 보안 설정 변경 0회, 다른 실행 경로로 재시도하지 않음 |
| 현재 PC 보존 | 실행 전 정상 종료 후 기록 확인. 거절된 업데이트 전후 앱 해시·기존 DB 행·위치 설정 동일. 기존 a3f891b 앱을 다시 실행해 1.707초에 통신 준비 확인. 새 수정본의 설치/연결 성공으로 계산하지 않음 |
| AI 시험 | 추가 0턴, 기존 누계 25/45. 설치 연결이 준비되기 전 일반 입력 시험이나 하위 작업을 실행하지 않음 |

이전 앱의 첫 재시작에서는 과거 제출 관측과 연결 확인 필드만 기존 `WorkflowStore::new` 정책대로 무효화됐다. 저장된 작업·계획·선호는 유지됐고 이후 거절된 업데이트에서는 추가 변경이 없었다. 이를 데이터 손실이나 모든 파일의 바이트 동일성으로 혼동하지 않는다.

현재 PC 검증의 다음 조건은 Windows가 정상 실행을 허용하는 설치본 또는 사용자가 직접 결정한 보안 설정 변경이다. 후자는 공개 사용자 배포의 통과 증거가 아니다. 원본 차단 로그, 검증 파일, 백업, 동일 설치본 CI 결과, 다음 실행 위치는 Git 제외 `work/mvp-goal/20260927-installed-mvp/`에 보관한다.
