...
목표 변경: 원래 채팅 모델 변경이 아닌 **펫 전용 하위 작업 라우팅**. 출발 소스 `0bf1c46`, Windows runtime `0.155.0-alpha.16`. 실제 가용 목록에서 Luna/low, Sol/low·medium을 재확인했다. 추가 AI 0턴, 누계 13/30. 새 상한은 30이다. 화면 조작 0건.
...
| 전용 훅 | 새로운 관측 정의 7개와 기존 11개 | 공식 hooks/list 오류 0. 새 정의 untrusted, 기존 modified. 기존 정의 보존 | 사용자의 정확한 새 정의 검토 |
...
| 검사·배포 구성 | Node 전체 및 저장소 밖 패키지 import | 최종 전체 **195/195**, 관련 패키지/관측 검사 37건, 구조·문서 링크 검사 통과 | Windows 결과 별도 기록 |
