...
생성 직전에 `inspect`로 전체 `brief`, `style`, `pendingRevision`을 읽는다. 짧은 `guidance`에는 긴 조건이나 원문이 생략될 수 있으므로 이것만으로 생성하지 않는다. 사용자가 이미지 생성을 요청한 경우에만 현재 AI가 이미 제공하는 기본 이미지 생성 도구·스킬을 사용한다. 가능한 도구가 없으면 구성안·문구·프롬프트를 제공하고 생성은 미완료라고 밝힌다. 외부 유료 API, 별도 에이전트 실행, 새로운 로그인으로 우회하지 않는다.
...
생성된 실제 PNG를 열어 원문과 비교한다. 생성 프롬프트를 실제 이미지의 문구라고 간주하지 않는다. 보이는 문구를 옮긴 `renderedText`와 최신 `expectedRevision`을 JSON으로 저장한다. 읽을 수 없는 문구는 임의 복원하지 않고 검토가 필요하다고 알린다.
