...
impl Store {
    ...
    pub fn pet_link_request(&mut self, request: Request) -> Result<Option<Link>, String> {
        let target = match &request { Request::Read{target} | Request::Connect{target} | Request::Settings{target,..} | Request::Enable{target,..} | Request::Disconnect{target,..} | Request::Prepare{target,..} | Request::Report{target,..} => target };
        target.validate()?;
        ...
        let expected = match &request { Request::Settings{expected_revision,..} | Request::Enable{expected_revision,..} | Request::Disconnect{expected_revision,..} | Request::Prepare{expected_revision,..} | Request::Report{expected_revision,..} => *expected_revision, _=>unreachable!() };
        ...
        match request {
            ...
            Request::Prepare{request_id,profile,..} => {
                ...
                if let Some(run)=&link.run {
                    ...
                    if matches!(run.state,RunState::Requested|RunState::Working|RunState::Unknown|RunState::Returned) { return Err("previous-pet-run-unresolved".into()); }
                }
                ...
                link.run=Some(Run{id:request_id,settings_revision:link.revision,profile:profile.clone(),model:model.into(),effort:effort.into(),state:RunState::Requested,child_id:None,turn_id:None,observed_model:None,observed_effort:None,evidence:None,started_at:now_ms()});
            },
            Request::Report{request_id,receipt,..} => {
                if !link.connected || !link.enabled { return Err("pet-disabled-or-disconnected".into()); }
                let run=link.run.as_mut().ok_or("pet-run-not-found")?;
                if run.id!=request_id || run.settings_revision!=link.revision { return Err("pet-run-mismatch".into()); }
                match receipt {
                    ...
                    Receipt::Runtime{parent_id,child_id,turn_id,model,effort,completed} => {
                        ...
                        if matches!(run.state,RunState::Failed|RunState::Unknown) { return Err("pet-run-closed".into()); }
