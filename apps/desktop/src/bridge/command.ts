...
export async function command<T=void>(name:string,args?:Record<string,unknown>):Promise<T> {
  if(!isDesktop) { if(name==='get_snapshot')return empty as T; if(name==='get_assistance')return EMPTY_ASSISTANCE as T; if(name==='workflow_snapshot')return EMPTY_WORKFLOW as T; throw new Error('Windows 데스크톱 앱에서 사용할 수 있습니다.'); }
