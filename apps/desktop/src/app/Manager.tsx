...
export function Manager({ snapshot, error, assistance, workflow }: { snapshot: Snapshot; error: string; assistance: ReturnType<typeof useAssistance>; workflow: ReturnType<typeof useWorkflow> }) {
  ...
  return <AppShell settings={settings} bound={bound} disconnected={disconnected} busy={action.busy} error={error || action.error} onNavigate={setSettings} onQuit={() => void action.run('quit_app')} dialogs={<>
    ...
    {detailSession && <Modal title={detailSession.label} close={() => setDetailId(null)} wide><p className="overlay-path">{shortPath(detailSession.cwd)}</p><TaskDetails session={detailSession} now={snapshot.now} disconnected={disconnected} /></Modal>}
      ...
      {settings === 'assistance' ? <AssistancePanel key={assistanceRequest} snapshot={assistance.snapshot} sessions={snapshot.sessions} initialSessionId={assistanceSession} error={assistance.error} loaded={assistance.loaded} refresh={assistance.refresh} workflow={workflow} /> : settings === 'connection' ? <ConnectionGuide setup={snapshot.setup} connectionPath={snapshot.connectionPath} onBack={() => setSettings('pets')} /> : <><div className="page-heading"><span className="eyebrow">A LITTLE COMPANY, A LITTLE CLARITY</span><h1>작업은 맡기고,<br />흐름은 가까이 두세요<span className="heading-spark" aria-hidden="true">✳</span></h1><p>펫 하나에 작업 하나. 실제 활동과 필요한 알림을 한눈에.</p></div><div className="section-title"><h2>나의 펫 <span>{bound} / 3</span></h2><button className="text-button" disabled={!isDesktop} onClick={() => void action.run('set_pets_visible', { visible: true })}>바탕화면에 모두 표시 ↗</button></div>
