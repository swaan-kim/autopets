...
import { STATUS, hookCommands, updateHooks } from '../integrations/codex/assistance/setup.mjs';

const setup = fileURLToPath(new URL('../integrations/codex/assistance/setup.mjs', import.meta.url));
...
test('assistance hook Windows command quotes Unicode and apostrophes without shell expansion', async t => {
  ...
  assert.equal(source, `& ${[process.execPath, fileURLToPath(new URL('../integrations/codex/assistance/prepare.mjs', import.meta.url)), 'hook', '--config', f.configFile].map(quote).join(' ')}; exit $LASTEXITCODE`);
});
