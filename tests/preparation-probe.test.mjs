...
import { handleHook, acknowledge, loadConfig, identityKey, validateContext, renderInjection, MAX_INJECTION_BYTES } from '../integrations/codex/probe/prepare.mjs';
import { configureHooks, commands, STATUS } from '../integrations/codex/probe/setup.mjs';

const entry = fileURLToPath(new URL('../integrations/codex/probe/prepare.mjs', import.meta.url));
const setup = fileURLToPath(new URL('../integrations/codex/probe/setup.mjs', import.meta.url));
...
test('M1 two chats retain isolated context and stdout is not called delivery proof', async t => {
  ...
  const status = await child(fileURLToPath(new URL('../integrations/codex/probe/status.mjs', import.meta.url)), ['--config', f.cfg.file]);
