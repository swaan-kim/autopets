...
const adapter = path.join(root, 'hooks', 'codex-hook.mjs');
const installer = path.join(root, 'scripts', 'install-hooks.mjs');
const checker = path.join(root, 'scripts', 'check-hooks.mjs');
...
const taskCli = path.join(root, 'skills', 'autopets', 'scripts', 'connect.mjs');
...
test('distributed skill scripts work independently of repository paths', async (t) => {
  ...
  for (const name of ['connect.mjs', 'bridge-client.mjs']) {
    await writeFile(path.join(standalone, name), await readFile(path.join(root, 'skills', 'autopets', 'scripts', name)));
  }
