...
import { defaultPreferences, utf8Bytes } from '../packages/contracts/index.mjs';
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) main().catch(error => { console.error(`Evaluation failed: ${error.message}`); process.exitCode = 1; });
