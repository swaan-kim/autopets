...
export async function packageChatGPT({out=join(root,'release/chatgpt'),node=null,nodeLicense=null}={}){
 ...
 for(const part of ['guidance','contracts']){const directory=join(root,'packages',part);try{await stat(directory);for(const f of await list(directory))extension.push({name:'vendor/'+part+'/'+f.name,data:f.data});}catch(error){if(error.code!=='ENOENT')throw error;}}
 const companion=[];
 for(const name of ['integrations/chatgpt/native/host.mjs','integrations/chatgpt/native/framing.mjs','integrations/chatgpt/native/install.mjs','integrations/chatgpt/extension/protocol.mjs','skills/autopets/scripts/bridge-client.mjs','integrations/chatgpt/README.md'])companion.push({name,data:await readFile(join(root,name))});
