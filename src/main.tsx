...
function ConfigurationForm({ session, submitLabel, busy, onSave }: { session: Session; submitLabel: string; busy: boolean; onSave: (config: TaskConfiguration) => Promise<void> }) {
  ...
  return <form className="configuration-form" onSubmit={event => { event.preventDefault(); if (config.completionCriterion.trim()) void onSave({ ...config, completionCriterion: config.completionCriterion.trim() }); }}>
    ...
    <label className="field-label" htmlFor="completion-criterion">어떤 결과가 나오면 끝난 건가요?</label>
    <textarea id="completion-criterion" className="text-input" required maxLength={2000} rows={3} placeholder="예: 참고 링크가 포함된 비교 보고서 초안 완성" value={config.completionCriterion} onChange={event => setConfig({ ...config, completionCriterion: event.target.value })} />
    ...
    <button className="button primary form-submit" disabled={busy || !config.completionCriterion.trim()}>{busy ? '저장 중…' : submitLabel}</button>
  </form>;
}

function PetOverlay({ snapshot, index, error }: { snapshot: Snapshot; index: number; error: string }) {
  ...
  return <div className={`pet-overlay ${expanded ? 'expanded' : ''}`}>
    {expanded && session && <section className="overlay-panel"><header><div><span className="eyebrow">{PET_NAMES[index]}의 작업</span><h2>{session.label}</h2></div><button className="icon-button" aria-label="카드 접기" onClick={() => setExpanded(false)}>×</button></header>
      <p className="overlay-path" title={session.cwd}>{shortPath(session.cwd)}</p>{(error || action.error || windowError) && <p className="error" role="alert">{error || action.error || windowError}</p>}<TaskDetails session={session} now={snapshot.now} disconnected={disconnected} /><button className="text-button" onClick={() => void action.run('show_manager')}>작업 설정 열기 ↗</button>
    </section>}
    <div className="floating-pet"><button className="drag-handle" aria-label="펫 이동" title="드래그해서 이동" onPointerDown={event => { if (event.button === 0 && isDesktop) void getCurrentWindow().startDragging().catch(() => void 0); }}>⠿</button>
      <button className="pet-hit" aria-expanded={expanded} aria-label={session ? `${session.label} · ${view.text}` : `${PET_NAMES[index]} · 작업 연결`} onClick={() => session ? setExpanded(!expanded) : void action.run('show_manager')}><Pet index={index} activity={observedActivity(session, disconnected)} paused={disconnected || session?.connection === 'unknown'} />{attention && <span className={`pet-attention-dot ${attention.kind}`} aria-label={view.text}>{attention.kind === 'elapsed' ? '◷' : attention.kind === 'milestone' ? '✓' : '!'}</span>}</button>
...
function Manager({ snapshot, error }: { snapshot: Snapshot; error: string }) {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark" aria-hidden="true">a<span>••</span></span><span>AutoPets<span className="brand-sub">작은 작업 동료</span></span></div><div className="nav-label">WORKSPACE</div><button className={`nav-item ${!settings ? 'active' : ''}`} onClick={() => setSettings(false)}><span aria-hidden="true">◈</span><span className="nav-text">나의 펫</span><span className="nav-count">{bound}/3</span></button><button className={`nav-item ${settings ? 'active' : ''}`} onClick={() => setSettings(true)}><span aria-hidden="true">⌘</span><span className="nav-text">연결 설정</span></button><div className="sidebar-bottom"><span className="local-indicator" />이 PC에서만 실행<p>작업의 흐름을 가까이,<br />필요한 순간에 확인해요.</p><span className="version">LOCAL MVP · 0.1</span></div></aside>
...
function App() {
  ...
  if (!snapshot) return <div className="loading-state" role="status">{error ? `브리지 연결 확인 필요 · ${error}` : '로컬 작업 상태를 불러오는 중…'}</div>;
  return petIndex === null ? <Manager snapshot={snapshot} error={error} /> : <PetOverlay snapshot={snapshot} index={petIndex} error={error} />;
}
