...
function ContextEditor({ task, disabled, onSave, onDelete }: { task: AssistanceTask; disabled: boolean; onSave: (context: TaskContext, revision: number) => Promise<boolean>; onDelete: () => Promise<boolean> }) {
...
export function AssistancePanel({ snapshot, sessions, initialSessionId, error, loaded, refresh }: { snapshot: AssistanceSnapshot; sessions: { id: string; label: string }[]; initialSessionId?: string | null; error: string; loaded: boolean; refresh: () => Promise<void> }) {
  ...
  return <section className="assistance-page" aria-label="자동 도움 설정">
    <div className="page-heading"><span className="eyebrow">YOUR WORK, YOUR WAY</span><h1>설정은 가볍게,<br />일은 내 방식대로.</h1><p>평소처럼 요청하세요. 연결이 확인된 도움만 적용해요.</p></div>
    ...
    <section className="assistance-card"><div className="block-heading"><div><h2>나를 위한 기본 설정</h2><p className="small-note">모든 채팅에는 선호만 공유해요. 업무 내용은 각각 보관해요.</p></div><span className={`status-badge ${snapshot.preferences.enabled ? 'working' : 'empty'}`}>{snapshot.preferences.enabled ? '자동 도움 켜짐' : '자동 도움 꺼짐'}</span></div>
      {!snapshot.preferences.enabled && <button className="button primary" disabled={disabled} onClick={() => void savePreferences({ ...snapshot.preferences, enabled: true, workStyle: 'auto', routingMode: 'auto', fixedModel: null, allowedModels: [], allowEscalation: false })}>추천 설정으로 켜기</button>}
      <form onSubmit={event => { event.preventDefault(); void savePreferences(draft); }}>
        ...
        <fieldset className="mode-fieldset"><legend>작업 방식</legend><div className="preference-choices">{([['auto', '자동', '필요한 만큼 준비·확인'], ['fast', '빠르게', '핵심 결과부터'], ['thorough', '꼼꼼하게', '근거와 누락 점검']] as const).map(([value, title, subtitle]) => <label key={value} className={`mode-option ${draft.workStyle === value ? 'selected' : ''}`}><input type="radio" name="work-style" value={value} checked={draft.workStyle === value} disabled={disabled} onChange={() => update({ ...draft, workStyle: value })} /><span><strong>{title}</strong><small>{subtitle}</small></span></label>)}</div></fieldset>
        ...
        <details className="assistance-details"><summary>상세 설정</summary><label>고정할 모델 ID<input className="text-input" placeholder="직접 선택한 모델 ID" value={draft.fixedModel || ''} disabled={disabled} onChange={event => update({ ...draft, fixedModel: event.target.value || null })} /></label><label>허용할 모델 ID<textarea aria-label="허용할 모델 ID" className="text-input" rows={2} placeholder="한 줄에 하나 · 비워두면 검증된 기본 범위" value={draft.allowedModels.join('\n')} disabled={disabled} onChange={event => update({ ...draft, allowedModels: event.target.value.split('\n') })} /></label><label className="assistance-toggle"><span><strong>더 높은 사용 수준 허용</strong><small>검증된 연결과 평가된 모델에만 적용</small></span><input type="checkbox" checked={draft.allowEscalation} disabled={disabled} onChange={event => update({ ...draft, allowEscalation: event.target.checked })} /></label><p className="small-note">입력한 ID가 실제 제공되는 모델인지는 연결에서 확인해요. 아직 확인되지 않은 모델로 자동 전환하지 않아요.</p></details>
    ...
    <section className="assistance-card"><h2>현재 채팅의 도움</h2>{snapshot.tasks.length ? <><label className="field-label" htmlFor="assistance-task">채팅 선택</label><select id="assistance-task" className="text-input" value={selected} onChange={event => { setSelected(event.target.value); setNotice(''); }}><option value="">채팅을 선택하세요</option>{snapshot.tasks.map(task => <option key={identityKey(task.identity)} value={identityKey(task.identity)}>{task.identity.provider === 'codex' ? 'Codex' : 'ChatGPT'} · {label(task)} · {task.identity.accountId}</option>)}</select>{selectedTask && <div className="chat-assistance" key={identityKey(selectedTask.identity)}><div className="chat-assistance-status"><span className={`status-badge ${selectedTask.assistance.status}`} data-testid="assistance-state">{assistanceLabel(selectedTask, snapshot.preferences.enabled)}</span><button className="text-button" disabled={disabled} onClick={() => void run('set_chat_assistance', { identity: selectedTask.identity, enabled: !selectedTask.enabled }, selectedTask.enabled ? '이번 채팅의 자동 도움을 껐어요. 기록은 남아 있어요.' : '이번 채팅의 자동 도움을 켰어요. 전달 상태를 확인해주세요.')}>{selectedTask.enabled ? '이번 채팅 도움 끄기' : '이번 채팅 도움 켜기'}</button></div><p>{selectedTask.assistance.reason || '아직 적용된 도움 내역이 없어요.'}</p><dl className="return-details"><dt>요청한 모델</dt><dd>{selectedTask.assistance.requestedModel || '기존 설정 유지'}</dd><dt>실제 적용 모델</dt><dd data-testid="applied-model">{selectedTask.assistance.appliedModel || '확인되지 않음'}</dd><dt>결과 점검</dt><dd>{selectedTask.quality.status === 'unchecked' ? '아직 점검하지 않음' : selectedTask.quality.status === 'needs-review' ? '확인할 항목 있음' : '등록된 점검 기준 통과'}</dd></dl>{selectedTask.quality.findings.length > 0 && <ul className="quality-findings">{selectedTask.quality.findings.map((finding, index) => <li key={index}>{finding}</li>)}</ul>}<ContextEditor task={selectedTask} disabled={disabled} onSave={(context, expectedRevision) => run('save_task_context', { identity: selectedTask.identity, context, expectedRevision }, '이 채팅의 기록을 저장했어요. AI 전달은 아직 확인되지 않았어요.')} onDelete={() => run('delete_task_context', { identity: selectedTask.identity }, '이 채팅의 저장 기록을 삭제했어요.')} /></div>}</> : <p className="unavailable-copy">연결이 확인된 채팅이 아직 없어요.<br /><span>채팅이 연결되면 도움과 전달 상태가 여기에 나타나요.</span></p>}</section>
    ...
    <section className="assistance-data"><button className="text-button danger-text" disabled={disabled || !snapshot.tasks.length} onClick={() => setConfirmAll(!confirmAll)}>모든 채팅 기록 삭제</button>{confirmAll && <div className="delete-confirm"><p>AutoPets에 저장한 모든 채팅 기록을 지워요. AI 서비스의 대화는 삭제하지 않아요.</p><button className="button secondary" disabled={disabled} onClick={() => setConfirmAll(false)}>취소</button><button className="button danger-button" disabled={disabled} onClick={async () => { if (await run('delete_all_contexts', {}, '모든 채팅의 저장 기록을 삭제했어요.')) setConfirmAll(false); }}>모든 기록 삭제 확인</button></div>}</section>
  </section>;
}
